...
def run_inference(args: argparse.Namespace, gpu_num: int, gpu_no: int) -> None:
    ...
    print(">>> torch.compile disabled for stability (using eager mode).")
            ...
            for idx in range(0, len(df)):
                ...
                for fs in args.frame_stride:
                    ...
                    for itr in tqdm(range(args.n_iter)):
                        ...
                        for action_idx in range(len(pred_actions[0])):
                            observation = {
                                'action': pred_actions[0][action_idx:action_idx + 1]
                            }
                        ...
                        for state_idx in range(args.exe_steps):
                            observation = {
                                'observation.images.top':
                                pred_videos_1[0][:, state_idx:state_idx +
                                                1].permute(1, 0, 2, 3),
                                'observation.state':
                                torch.zeros_like(pred_states[0][state_idx:state_idx + 1])
                                if args.zero_pred_state else
                                pred_states[0][state_idx:state_idx + 1],
                                'action':
